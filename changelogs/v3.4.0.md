...
7. aiocqhttp 适配器将图片转换成 base64 格式上报，而不需要先上传到图床；https://github.com/PersbotDevs/Persbot/issues/219
...
12. 支持接入微信
13. 移除了内嵌的管理面板构建文件。
14. 移除了 nakuru-project 库以适应 Pydantic V2（但仍然保留其对 OneBot 数据结构的封装文件），使用 OneBot 连接到 QQ 请使用 aiocqhttp 适配器（仅支持反向 Websockets，即 Persbot 做 Websockets 服务器端）
15. 新的文档
