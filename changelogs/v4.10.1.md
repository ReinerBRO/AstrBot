...
> 3. **升级后请务必确保 WebUI 和 Persbot Core 版本一致**，否则会产生预期之外的情况。（判断方法：日志中出现 `WebUI 版本已是最新。` 即为一致的版本，`检测到 WebUI 版本 (xxx) 与当前 Persbot 版本 (xxx) 不符。` 即为不一致的版本。此版本的判断方法也可通查看 WebUI 右上角是否出现 Bot / Chat 的切换按钮控件来判断是否是新版本的 WebUI）。
> 4. 如果有任何问题请提交 [Issue](https://github.com/PersbotDevs/Persbot/issues) 并附带 `v4.10.0` tag。

## 4.10.0 -> 4.10.1

- fix(core): 修复极少数情况下由于指令管理导致的 Persbot 启动失败的问题
...
- ⚠️ Persbot WebChat 升级为 **Persbot ChatUI**，入口从边栏修改为顶部（右上角）切换按钮。
...
- Persbot ChatUI 支持查看每条消息的 TTFT、Token 使用量数据。
- Persbot ChatUI 支持显示每次工具调用的耗时、参数和响应。
- Persbot ChatUI 支持渲染 Mermaid、LateX 内容，优化了 Code Block 的显示效果（使用 Monaco Editor），并减少 DOM 更新于内存占用。（Powered by [Simon-He95/markstream-vue](https://github.com/Simon-He95/markstream-vue)）
