...
export function useI18n() {
  ...
  const setLocale = async (newLocale: Locale) => {
    if (newLocale !== currentLocale.value) {
      ...
      // 保存到localStorage
      localStorage.setItem('persbot-locale', newLocale);

      // 触发自定义事件，通知相关页面重新加载配置数据
      // 这是因为插件适配器的 i18n 数据是通过后端 API 注入的，
      // 需要根据 Accept-Language 头重新获取
      window.dispatchEvent(new CustomEvent('persbot-locale-changed', {
...
export async function setupI18n() {
  ...
  const savedLocale = localStorage.getItem('persbot-locale') as Locale;
