...
from persbot.core.star.star_handler import StarHandlerMetadata, star_handlers_registry
from persbot.core.utils.command_parser import CommandParserMixin
...
class AlterCmdCommands(CommandParserMixin):
    ...
    async def alter_cmd(self, event: AstrMessageEvent) -> None:
        ...
        if cmd_name == "reset" and cmd_type == "config":
            ...
            alter_cmd_cfg = await sp.global_get("alter_cmd", {})
            plugin_ = alter_cmd_cfg.get("persbot", {})
        ...
        for filter_ in found_command.event_filters:
            if isinstance(filter_, PermissionTypeFilter):
                if cmd_type == "admin":
                    from persbot.api.event import filter
