...
from persbot.core.star import command_management
...
class HelpCommand:
    ...
    async def _query_persbot_notice(self):
        try:
            async with aiohttp.ClientSession(trust_env=True) as session:
                async with session.get(
                    "https://persbot.app/notice.json",
                    timeout=2,
                ) as resp:
    ...
    async def help(self, event: AstrMessageEvent) -> None:
        ...
        notice = ""
        try:
            notice = await self._query_persbot_notice()
        except BaseException:
            pass
        ...
        msg_parts = [
            f"Persbot v{VERSION}(WebUI: {dashboard_version})",
            "内置指令:",
            commands_section,
        ]
        if notice:
