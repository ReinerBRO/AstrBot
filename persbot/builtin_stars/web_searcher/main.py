...
from persbot.api import PersbotConfig, llm_tool, logger, sp, star
from persbot.api.event import AstrMessageEvent, filter
from persbot.api.provider import ProviderRequest
from persbot.core.provider.func_tool_manager import FunctionToolManager
...
class Main(star.Star):
    ...
    async def _get_tavily_key(self, cfg: PersbotConfig) -> str:
        """并发安全的从列表中获取并轮换Tavily API密钥。"""
        tavily_keys = cfg.get("provider_settings", {}).get("websearch_tavily_key", [])
        if not tavily_keys:
            raise ValueError("错误：Tavily API密钥未在Persbot中配置。")
    ...
    async def _extract_tavily(self, cfg: PersbotConfig, payload: dict) -> list[dict]:
        """使用 Tavily 提取网页内容"""
        tavily_key = await self._get_tavily_key(cfg)
    ...
    async def ensure_baidu_ai_search_mcp(self, umo: str | None = None) -> None:
        ...
        if not key:
            raise ValueError(
                "Error: Baidu AI Search API key is not configured in Persbot.",
            )
    ...
    async def search_from_tavily(
        ...
        if not cfg.get("provider_settings", {}).get("websearch_tavily_key", []):
            raise ValueError("Error: Tavily API key is not configured in Persbot.")
    ...
    async def _get_bocha_key(self, cfg: PersbotConfig) -> str:
        """并发安全的从列表中获取并轮换BoCha API密钥。"""
        bocha_keys = cfg.get("provider_settings", {}).get("websearch_bocha_key", [])
        if not bocha_keys:
            raise ValueError("错误：BoCha API密钥未在Persbot中配置。")
    ...
    async def search_from_bocha(
        ...
        if not cfg.get("provider_settings", {}).get("websearch_bocha_key", []):
            raise ValueError("Error: BoCha API key is not configured in Persbot.")
