...
def _get_data_path() -> Path:
    base = get_persbot_root()
    if not check_persbot_root(base):
        raise click.ClickException(
            f"{base} is not a valid Persbot root directory. Use 'persbot init' to initialize",
        )
...
def new(name: str) -> None:
    ...
    with open(plug_path / "README.md", "w", encoding="utf-8") as f:
        f.write(
            f"# {name}\n\n{desc}\n\n# Support\n\n[Documentation](https://persbot.app)\n"
        )
