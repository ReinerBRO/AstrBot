...
from ..utils import check_persbot_root, get_persbot_root
...
def _load_config() -> dict[str, Any]:
    ...
    root = get_persbot_root()
    if not check_persbot_root(root):
        raise click.ClickException(
            f"{root} is not a valid Persbot root directory. Use 'persbot init' to initialize",
        )
...
def _save_config(config: dict[str, Any]) -> None:
    """Save config file"""
    config_path = get_persbot_root() / "data" / "cmd_config.json"
