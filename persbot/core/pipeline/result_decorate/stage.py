...
from persbot.core import file_token_service, html_renderer, logger
from persbot.core.message.components import At, Image, Node, Plain, Record, Reply
from persbot.core.message.message_event_result import ResultContentType
from persbot.core.pipeline.content_safety_check.stage import ContentSafetyCheckStage
...
from persbot.core.star.session_llm_manager import SessionServiceManager
from persbot.core.star.star import star_map
from persbot.core.star.star_handler import EventType, star_handlers_registry
...
class ResultDecorateStage(Stage):
    async def initialize(self, ctx: PipelineContext) -> None:
        self.ctx = ctx
        self.reply_prefix = ctx.persbot_config["platform_settings"]["reply_prefix"]
        self.reply_with_mention = ctx.persbot_config["platform_settings"][
            "reply_with_mention"
        ]
        self.reply_with_quote = ctx.persbot_config["platform_settings"][
            "reply_with_quote"
        ]
        self.t2i_word_threshold = ctx.persbot_config["t2i_word_threshold"]
        try:
            self.t2i_word_threshold = int(self.t2i_word_threshold)
            self.t2i_word_threshold = max(self.t2i_word_threshold, 50)
        except BaseException:
            self.t2i_word_threshold = 150
        self.t2i_strategy = ctx.persbot_config["t2i_strategy"]
        self.t2i_use_network = self.t2i_strategy == "remote"
        self.t2i_active_template = ctx.persbot_config["t2i_active_template"]

        self.forward_threshold = ctx.persbot_config["platform_settings"][
            "forward_threshold"
        ]

        trigger_probability = ctx.persbot_config["provider_tts_settings"].get(
            "trigger_probability",
            1,
        )
        try:
        ...
        self.words_count_threshold = int(
            ctx.persbot_config["platform_settings"]["segmented_reply"][
                "words_count_threshold"
            ],
        )
        self.enable_segmented_reply = ctx.persbot_config["platform_settings"][
            "segmented_reply"
        ]["enable"]
        self.only_llm_result = ctx.persbot_config["platform_settings"][
            "segmented_reply"
        ]["only_llm_result"]
        self.split_mode = ctx.persbot_config["platform_settings"][
            "segmented_reply"
        ].get("split_mode", "regex")
        self.regex = ctx.persbot_config["platform_settings"]["segmented_reply"]["regex"]
        self.split_words = ctx.persbot_config["platform_settings"][
            "segmented_reply"
        ].get("split_words", ["。", "？", "！", "~", "…"])
        if self.split_words:
        ...
        self.content_cleanup_rule = ctx.persbot_config["platform_settings"][
            "segmented_reply"
        ]["content_cleanup_rule"]

        # exception
        self.content_safe_check_reply = ctx.persbot_config["content_safety"][
        ...
        provider_cfg = ctx.persbot_config.get("provider_settings", {})
    ...
    async def process(
        ...
        if len(result.chain) > 0:
            ...
            should_tts = (
                bool(self.ctx.persbot_config["provider_tts_settings"]["enable"])
                and result.is_llm_result()
                ...
                for comp in result.chain:
                    if isinstance(comp, Plain) and len(comp.text) > 1:
                        try:
                            ...
                            use_file_service = self.ctx.persbot_config[
                                "provider_tts_settings"
                            ]["use_file_service"]
                            callback_api_base = self.ctx.persbot_config[
                                "callback_api_base"
                            ]
                            dual_output = self.ctx.persbot_config[
                                "provider_tts_settings"
                            ]["dual_output"]
            ...
            # 文本转图片
                ...
                result.use_t2i_ is None and self.ctx.persbot_config["t2i"]
            ) or result.use_t2i_:
                parts = []
                for comp in result.chain:
                    if isinstance(comp, Plain):
                ...
                if plain_str and len(plain_str) > self.t2i_word_threshold:
                    ...
                    if url:
                        if url.startswith("http"):
                            ...
                            self.ctx.persbot_config["t2i_use_file_service"]
                            and self.ctx.persbot_config["callback_api_base"]
                        ):
                            token = await file_token_service.register_file(url)
                            url = f"{self.ctx.persbot_config['callback_api_base']}/api/file/{token}"
                            logger.debug(f"已注册：{url}")
                            result.chain = [Image.fromURL(url)]
                        else:
