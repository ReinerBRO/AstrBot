...
from persbot.core.message.components import At, AtAll, Reply
from persbot.core.message.message_event_result import MessageChain, MessageEventResult
...
from persbot.core.star.filter.permission import PermissionTypeFilter
from persbot.core.star.session_plugin_manager import SessionPluginManager
...
class WakingCheckStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        ...
        """
        self.ctx = ctx
        self.no_permission_reply = self.ctx.persbot_config["platform_settings"].get(
            "no_permission_reply",
            True,
        )
        # 私聊是否需要 wake_prefix 才能唤醒机器人
        self.friend_message_needs_wake_prefix = self.ctx.persbot_config[
            "platform_settings"
        ].get("friend_message_needs_wake_prefix", False)
        # 是否忽略机器人自己发送的消息
        self.ignore_bot_self_message = self.ctx.persbot_config["platform_settings"].get(
            "ignore_bot_self_message",
            False,
        )
        self.ignore_at_all = self.ctx.persbot_config["platform_settings"].get(
            "ignore_at_all",
            False,
        )
        self.disable_builtin_commands = self.ctx.persbot_config.get(
            "disable_builtin_commands", False
        )
        platform_settings = self.ctx.persbot_config.get("platform_settings", {})
    ...
    async def process(
        ...
        for admin_id in self.ctx.persbot_config["admins_id"]:
            if str(event.get_sender_id()) == admin_id:
                event.role = "admin"
                break

        # 检查 wake
        wake_prefixes = self.ctx.persbot_config["wake_prefix"]
        ...
        enabled_plugins_name = self.ctx.persbot_config.get("plugin_set", ["*"])
        ...
        for handler in star_handlers_registry.get_handlers_by_event_type(
            ...
            if (
                ...
                == "persbot.builtin_stars.builtin_commands.main"
            ):
                continue
            ...
            for filter in handler.event_filters:
                try:
                    if isinstance(filter, PermissionTypeFilter):
                        if not filter.filter(event, self.ctx.persbot_config):
                            permission_not_pass = True
                            permission_filter_raise_error = filter.raise_error
                    elif not filter.filter(event, self.ctx.persbot_config):
                        passed = False
                        break
