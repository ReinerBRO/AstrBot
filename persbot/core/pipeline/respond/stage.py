...
from persbot.core.message.components import BaseMessageComponent, ComponentType
from persbot.core.message.message_event_result import MessageChain, ResultContentType
from persbot.core.platform.astr_message_event import AstrMessageEvent
from persbot.core.star.star_handler import EventType
from persbot.core.utils.path_util import path_Mapping
...
class RespondStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        self.ctx = ctx
        self.config = ctx.persbot_config
        ...
        # 分段回复
        self.enable_seg: bool = ctx.persbot_config["platform_settings"][
            "segmented_reply"
        ]["enable"]
        ...
        self.interval_method = ctx.persbot_config["platform_settings"][
            "segmented_reply"
        ]["interval_method"]
        self.log_base = float(
            ctx.persbot_config["platform_settings"]["segmented_reply"]["log_base"],
        )
        self.interval = [1.5, 3.5]
        if self.enable_seg:
            interval_str: str = ctx.persbot_config["platform_settings"][
                "segmented_reply"
            ]["interval"]
