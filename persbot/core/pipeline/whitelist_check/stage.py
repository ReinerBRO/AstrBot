...
class WhitelistCheckStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        self.enable_whitelist_check = ctx.persbot_config["platform_settings"][
            "enable_id_white_list"
        ]
        self.whitelist = ctx.persbot_config["platform_settings"]["id_whitelist"]
        self.whitelist = [
            str(i).strip() for i in self.whitelist if str(i).strip() != ""
        ]
        self.wl_ignore_admin_on_group = ctx.persbot_config["platform_settings"][
            "wl_ignore_admin_on_group"
        ]
        self.wl_ignore_admin_on_friend = ctx.persbot_config["platform_settings"][
            "wl_ignore_admin_on_friend"
        ]
        self.wl_log = ctx.persbot_config["platform_settings"]["id_whitelist_log"]
