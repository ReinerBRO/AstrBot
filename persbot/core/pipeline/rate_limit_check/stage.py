...
from persbot.core.config.persbot_config import RateLimitStrategy
...
class RateLimitStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        """初始化限流器，根据配置设置限流参数。"""
        self.rate_limit_count = ctx.persbot_config["platform_settings"]["rate_limit"][
            "count"
        ]
        self.rate_limit_time = timedelta(
            seconds=ctx.persbot_config["platform_settings"]["rate_limit"]["time"],
        )
        self.rl_strategy = ctx.persbot_config["platform_settings"]["rate_limit"][
            "strategy"
        ]  # stall or discard
