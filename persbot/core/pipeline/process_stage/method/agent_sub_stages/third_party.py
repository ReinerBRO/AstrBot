...
from persbot.core.agent.runners.deerflow.deerflow_agent_runner import (
    DeerFlowAgentRunner,
)
from persbot.core.agent.runners.dify.dify_agent_runner import DifyAgentRunner
...
if TYPE_CHECKING:
    from persbot.core.agent.runners.base import BaseAgentRunner
    from persbot.core.provider.entities import LLMResponse
from persbot.core.pipeline.stage import Stage
...
from persbot.core.utils.config_number import coerce_int_config
...
class ThirdPartyAgentSubStage(Stage):
    async def initialize(self, ctx: PipelineContext) -> None:
        ...
        settings = ctx.persbot_config["provider_settings"]
    ...
    async def process(
        ...
        self.prov_cfg: dict = next(
            (p for p in persbot_config["provider"] if p["id"] == self.prov_id),
            {},
        )
        if not self.prov_id:
