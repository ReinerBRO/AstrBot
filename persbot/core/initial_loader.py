"""Persbot 启动器，负责初始化和启动核心组件和仪表板服务器。
...
from persbot.core import LogBroker, logger
...
class InitialLoader:
    ...
    async def start(self) -> None:
        core_lifecycle = PersbotCoreLifecycle(self.log_broker, self.db)

        try:
            await core_lifecycle.initialize()
        except Exception as e:
            logger.critical(traceback.format_exc())
            logger.critical(f"😭 初始化 Persbot 失败：{e} !!!")
            return
        ...
        self.dashboard_server = PersbotDashboard(
            core_lifecycle,
            self.db,
        ...
        if coro:
            ...
            task = core_task
        try:
            await task  # 整个Persbot在这里运行
        except asyncio.CancelledError:
            logger.info("🌈 正在关闭 Persbot...")
