...
def register_on_persbot_loaded(**kwargs):
    """当 Persbot 加载完成时"""

    def decorator(awaitable):
        _ = get_handler_or_create(awaitable, EventType.OnPersbotLoadedEvent, **kwargs)
        return awaitable
...
def register_llm_tool(name: str | None = None, **kwargs):
    ...
    请务必按照以下格式编写一个工具（包括函数注释，Persbot 会尝试解析该函数注释）
