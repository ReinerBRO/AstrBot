...
首先需要构造一个 PersbotMessage 对象, 使用 create_message 方法
...
from persbot.core.platform.sources.aiocqhttp.aiocqhttp_platform_adapter import (
    AiocqhttpAdapter,
)
...
class StarTools:
    ...
    async def create_event(
        ...
        Args:
            abm (PersbotMessage): 要提交的消息对象, 请先使用 create_message 创建
            platform (str): 可选的平台名称，默认平台(aiocqhttp), 目前只支持 aiocqhttp
    ...
    def get_data_dir(cls, plugin_name: str | None = None) -> Path:
        ...
        data_dir = Path(
            os.path.join(get_persbot_data_path(), "plugin_data", plugin_name),
        )
