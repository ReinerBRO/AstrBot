...
from persbot.core.provider.entities import LLMResponse, ProviderRequest, ProviderType
from persbot.core.provider.func_tool_manager import FunctionTool, FunctionToolManager
from persbot.core.provider.manager import ProviderManager
from persbot.core.provider.provider import (
    EmbeddingProvider,
    Provider,
    RerankProvider,
    STTProvider,
...
class Context:
    ...
    def __init__(
        ...
        persbot_config_mgr: PersbotConfigManager,
        ...
        """人格角色设定管理器"""
        self.persbot_config_mgr = persbot_config_mgr
        """配置文件管理器(非webui)"""
    ...
    async def tool_loop_agent(
        ...
        **kwargs: Any,
        ...
        from persbot.core.astr_agent_context import (
            AgentContextWrapper,
            AstrAgentContext,
        )
    ...
    def get_config(self, umo: str | None = None) -> PersbotConfig:
        """获取 Persbot 的配置。
        ...
        if not umo:
            # 使用默认配置
            return self._config
        return self.persbot_config_mgr.get_conf(umo)
    ...
    """
    以下的方法已经不推荐使用。请从 Persbot 文档查看更好的注册方式。
    """
    ...
    def get_platform(self, platform_type: PlatformAdapterType | str) -> Platform | None:
        ...
        Note:
            该方法已经过时，请使用 get_platform_inst 方法。(>= Persbot v4.0.0)
        """
        for platform in self.platform_manager.platform_insts:
