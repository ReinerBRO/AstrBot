...
try:
    ...
    if os.getenv("PERSBOT_RELOAD", "0") == "1":
        logger.warning("未安装 watchfiles，无法实现插件的热重载。")


class PluginVersionIncompatibleError(Exception):
    """Raised when plugin persbot_version is incompatible with current Persbot."""
...
def _temporary_filtered_requirements_file(
    *,
    ...
    temp_dir = get_persbot_temp_path()
...
class PluginManager:
    def __init__(self, context: Context, config: PersbotConfig) -> None:
        ...
        self.config = config
        self.plugin_store_path = get_persbot_plugin_path()
        """存储插件的路径。即 data/plugins"""
        self.plugin_config_path = get_persbot_config_path()
        """存储插件配置的路径。data/config"""
        self.reserved_plugin_path = os.path.join(
            get_persbot_path(), "persbot", "builtin_stars"
        )
        """保留插件的路径。在 persbot/builtin_stars 目录下"""
    ...
    def _load_plugin_metadata(plugin_path: str, plugin_obj=None) -> StarMetadata | None:
        ...
        Notes: 旧版本 Persbot 插件可能使用的是 info() 函数来获取元数据。
        """
        metadata = None
        ...
        if isinstance(metadata, dict):
            ...
            metadata = StarMetadata(
                ...
                persbot_version=(
                    metadata["persbot_version"]
                    if isinstance(metadata.get("persbot_version"), str)
                    else None
                ),
            )
    ...
    @staticmethod
    def _validate_persbot_version_specifier(
        ...
        try:
            ...
            return (
                False,
                "persbot_version 格式无效，请使用 PEP 440 版本范围格式，例如 >=4.16,<5。",
            )
        ...
        try:
            ...
            return (
                False,
                f"Persbot 当前版本 {VERSION} 无法被解析，无法校验插件版本范围。",
            )

        if current_version not in specifier:
            return (
                False,
                f"当前 Persbot 版本为 {VERSION}，不满足插件要求的 persbot_version: {normalized_spec}",
            )
        return True, None
    ...
    def _get_plugin_related_modules(
        ...
        """
        prefix = "persbot.builtin_stars." if is_reserved else "data.plugins."
        return [
            key
    ...
    def _purge_modules(
        ...
        Args:
            module_patterns: 要移除的模块名模式列表（例如 ["data.plugins", "persbot.builtin_stars"]）
                ...
                record.update(
                    {
                        ...
                        "persbot_version": metadata.persbot_version,
                    }
                )
    ...
    def _rebuild_failed_plugin_info(self) -> None:
        ...
        for dir_name, info in self.failed_plugin_dict.items():
            if isinstance(info, dict):
                ...
                version = info.get("version") or info.get("persbot_version")
                if version:
                    lines.append(
        ...
        for plugin_module in plugin_modules:
            try:
                ...
                reserved = plugin_module.get(
                    "reserved",
                    False,
                )  # 是否是保留插件。目前在 persbot/builtin_stars 目录下的都是保留插件。保留插件不可以卸载。
                plugin_dir_path = (
                ...
                path = "data.plugins." if not reserved else "persbot.builtin_stars."
                ...
                if path in star_map:
                    ...
                    try:
                        ...
                        if metadata_yaml:
                            ...
                            metadata.persbot_version = metadata_yaml.persbot_version
                    except Exception as e:
                        logger.warning(
                    ...
                    if not ignore_version_check:
                        is_valid, error_message = (
                            self._validate_persbot_version_specifier(
                                metadata.persbot_version,
                            )
                        )
                        if not is_valid:
                            raise PluginVersionIncompatibleError(
                                error_message
                                or "The plugin is not compatible with the current Persbot version."
                            )
    ...
    async def uninstall_plugin(
        ...
        async with self._pm_lock:
            ...
            if not plugin:
                raise Exception("插件不存在。")
            if plugin.reserved:
                raise Exception("该插件是 Persbot 保留插件，无法卸载。")
    ...
    async def _unbind_plugin(self, plugin_name: str, plugin_module_path: str) -> None:
        ...
        for func_tool in llm_tools.func_list:
            ...
            if (
                ...
                and not mp.endswith(("persbot.builtin_stars", "data.plugins"))
            ):
    ...
    async def update_plugin(self, plugin_name: str, proxy="") -> None:
        ...
        if not plugin:
            raise Exception("插件不存在。")
        if plugin.reserved:
            raise Exception("该插件是 Persbot 保留插件，无法更新。")
