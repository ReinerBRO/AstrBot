...
CMD_CONFIG_FILE_PATH = os.path.join(get_persbot_data_path(), "cmd_config.json")
KB_PATH = get_persbot_knowledge_base_path()
DEFAULT_PLATFORM_STATS_INVALID_COUNT_WARN_LIMIT = 5
PLATFORM_STATS_INVALID_COUNT_WARN_LIMIT_ENV = (
    "PERSBOT_PLATFORM_STATS_INVALID_COUNT_WARN_LIMIT"
)
...
class PersbotImporter:
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as zf:
                ...
                try:
                    ...
                    result.error = "备份文件缺少 manifest.json，不是有效的 Persbot 备份"
                    return result
                ...
                # 提取基本信息
                result.backup_version = manifest.get("persbot_version", "未知")
        ...
        if VersionComparator.compare_version(backup_major, current_major) != 0:
            return {
                ...
                "message": (
                    ...
                    f"跨主版本导入可能导致数据损坏，请使用相同主版本的 Persbot。"
                ),
            }
    ...
    def _validate_version(self, manifest: dict) -> None:
        ...
        """
        backup_version = manifest.get("persbot_version")
        if not backup_version:
    ...
    async def _import_kb_documents(self, kb_id: str, doc_data: dict) -> None:
        """导入知识库文档到向量数据库"""
        from persbot.core.db.vec_db.faiss_impl.document_storage import DocumentStorage
