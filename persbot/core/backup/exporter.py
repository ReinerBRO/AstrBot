...
class PersbotExporter:
    ...
    async def export_all(
        ...
        if output_dir is None:
            output_dir = get_persbot_backups_path()

        # 确保输出目录存在
        Path(output_dir).mkdir(parents=True, exist_ok=True)

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        zip_filename = f"persbot_backup_{timestamp}.zip"
    ...
    async def _export_kb_documents(self, kb_helper: Any) -> dict[str, Any]:
        """导出知识库的文档块数据"""
        try:
            from persbot.core.db.vec_db.faiss_impl.vec_db import FaissVecDB
