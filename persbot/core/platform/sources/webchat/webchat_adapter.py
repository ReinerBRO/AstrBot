...
class WebChatAdapter(Platform):
    def __init__(
        ...
        self.imgs_dir = os.path.join(get_persbot_data_path(), "webchat", "imgs")
        self.attachments_dir = Path(get_persbot_data_path()) / "attachments"
    ...
    async def convert_message(self, data: tuple) -> PersbotMessage:
    ...
    async def handle_msg(self, message: PersbotMessage) -> None:
