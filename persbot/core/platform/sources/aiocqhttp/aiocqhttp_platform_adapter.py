...
class AiocqhttpAdapter(Platform):
    ...
    async def convert_message(self, event: Event) -> PersbotMessage | None:
    ...
    async def _convert_handle_request_event(self, event: Event) -> PersbotMessage:
        """OneBot V11 请求类事件"""
        abm = PersbotMessage()
    ...
    async def _convert_handle_notice_event(self, event: Event) -> PersbotMessage:
        """OneBot V11 通知类事件"""
        abm = PersbotMessage()
        abm.self_id = str(event.self_id)
        abm.sender = MessageMember(
