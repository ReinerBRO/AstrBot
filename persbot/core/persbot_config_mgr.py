...
DEFAULT_CONFIG_CONF_INFO = ConfInfo(
    id="default",
    name="default",
    path=PERSBOT_CONFIG_PATH,
)


class PersbotConfigManager:
    """A class to manage the system configuration of Persbot, aka ACM"""

    def __init__(
        self,
        default_config: PersbotConfig,
        ucr: UmopConfigRouter,
        sp: SharedPreferences,
    ) -> None:
        self.sp = sp
        self.ucr = ucr
        self.confs: dict[str, PersbotConfig] = {}
        """uuid / "default" -> PersbotConfig"""
    ...
    def _load_all_configs(self) -> None:
        ...
        for uuid_, meta in abconf_data.items():
            filename = meta["path"]
            conf_path = os.path.join(get_persbot_config_path(), filename)
            if os.path.exists(conf_path):
                conf = PersbotConfig(config_path=conf_path)
    ...
    def get_conf(self, umo: str | MessageSession | None) -> PersbotConfig:
    ...
    @property
    def default_conf(self) -> PersbotConfig:
        """获取默认配置文件"""
        return self.confs["default"]
    ...
    def create_conf(
        ...
        conf_path = os.path.join(get_persbot_config_path(), conf_file_name)
        conf = PersbotConfig(config_path=conf_path, default_config=config)
        conf.save_config()
