...
from persbot.core.db.po import ConversationV2, PlatformMessageHistory, PlatformSession
...
async def migrate_webchat_session(db_helper: BaseDatabase) -> None:
    ...
    try:
        async with db_helper.get_db() as session:
            ...
            # 对于每个 user_id，对应的 conversation user_id 格式为: webchat:FriendMessage:webchat!persbot!{user_id}
            user_ids_to_query = [
                f"webchat:FriendMessage:webchat!persbot!{user_id}"
            ...
            title_map = {
                user_id.replace("webchat:FriendMessage:webchat!persbot!", ""): title
