...
class PlatformStat(SQLModel, table=True):
    ...
    Note: In persbot v4, we moved `platform` table to here.
    """
...
class Attachment(TimestampMixin, SQLModel, table=True):
    """This class represents attachments for messages in Persbot.
...
class Conversation:
    ...
    updated_at: int = 0
    token_usage: int = 0
    """对话的总 token 数量。Persbot 会保留最近一次 LLM 请求返回的总 token 数，方便统计。token_usage 可能为 0，表示未知。"""
