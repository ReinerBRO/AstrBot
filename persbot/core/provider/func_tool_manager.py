...
from persbot.core.agent.mcp_client import MCPClient, MCPTool
...
MCP_INIT_TIMEOUT_ENV = "PERSBOT_MCP_INIT_TIMEOUT"
ENABLE_MCP_TIMEOUT_ENV = "PERSBOT_MCP_ENABLE_TIMEOUT"
...
class FunctionToolManager:
    ...
    async def init_mcp_clients(
        ...
        Timeout behavior:
        - 初始化超时使用环境变量 PERSBOT_MCP_INIT_TIMEOUT 或默认值。
        - 动态启用超时使用 PERSBOT_MCP_ENABLE_TIMEOUT（独立于初始化超时）。
        """
    ...
    async def enable_mcp_server(
        ...
        Args:
            ...
            timeout: Timeout in seconds for initialization.
                Uses PERSBOT_MCP_ENABLE_TIMEOUT by default (separate from init timeout).
