...
from persbot.core.config import VERSION
from persbot.core.utils.http_ssl import build_tls_connector
from persbot.core.utils.io import download_image_by_url
from persbot.core.utils.t2i.template_manager import TemplateManager

from . import RenderStrategy

PERSBOT_T2I_DEFAULT_ENDPOINT = "https://t2i.soulter.top/text2img"
...
class NetworkRenderStrategy(RenderStrategy):
    def __init__(self, base_url: str | None = None) -> None:
        super().__init__()
        if not base_url:
            self.BASE_RENDER_URL = PERSBOT_T2I_DEFAULT_ENDPOINT
        else:
    ...
    async def initialize(self) -> None:
        if self.BASE_RENDER_URL == PERSBOT_T2I_DEFAULT_ENDPOINT:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    "https://api.soulter.top/persbot/t2i-endpoints",
                ) as resp:
                    if resp.status == 200:
