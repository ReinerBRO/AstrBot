...
from persbot.core.utils.io import save_temp_img
...
class FontManager:
    ...
    def get_font(cls, size: int) -> ImageFont.FreeTypeFont|ImageFont.ImageFont:
        ...
        # 首先尝试加载自定义字体
        try:
            font_path = os.path.join(get_persbot_data_path(), "font.ttf")
...
class MarkdownRenderer:
    ...
    async def render(self, markdown_text: str) -> Image.Image:
        ...
        # 绘制"Powered by Persbot"文本
        footer_font_size = 20
        footer_font = FontManager.get_font(footer_font_size)

        # 获取"Powered by "和"Persbot"的宽度以便居中
        powered_by_text = "Powered by "
        persbot_text = f"Persbot v{VERSION}"

        powered_by_width, _ = TextMeasurer.get_text_size(powered_by_text, footer_font)
        persbot_width, _ = TextMeasurer.get_text_size(persbot_text, footer_font)

        total_width = powered_by_width + persbot_width
