...
from .persbot_path import get_persbot_data_path, get_persbot_path, get_persbot_temp_path

logger = logging.getLogger("persbot")
...
async def get_dashboard_version():
    ...
    dist_dir = os.path.join(get_persbot_data_path(), "dist")
    if not os.path.exists(dist_dir):
        # Fall back to the dist bundled inside the installed wheel.
        _bundled = Path(get_persbot_path()) / "persbot" / "dashboard" / "dist"
        if _bundled.exists():
...
async def download_dashboard(
    ...
    """下载管理面板文件"""
    if path is None:
        zip_path = Path(get_persbot_data_path()).absolute() / "dashboard.zip"
    else:
        zip_path = Path(path).absolute()

    if latest or len(str(version)) != 40:
        ver_name = "latest" if latest else version
        dashboard_release_url = f"https://persbot-registry.soulter.top/download/persbot-dashboard/{ver_name}/dist.zip"
        logger.info(
            f"准备下载指定发行版本的 Persbot WebUI 文件: {dashboard_release_url}",
        )
        try:
            await download_file(
            ...
            if latest:
                dashboard_release_url = "https://github.com/PersbotDevs/Persbot/releases/latest/download/dist.zip"
            else:
                dashboard_release_url = f"https://github.com/PersbotDevs/Persbot/releases/download/{version}/dist.zip"
            if proxy:
        ...
        url = f"https://github.com/PersbotDevs/persbot-release-harbour/releases/download/release-{version}/dist.zip"
        logger.info(f"准备下载指定版本的 Persbot WebUI: {url}")
        if proxy:
            url = f"{proxy}/{url}"
