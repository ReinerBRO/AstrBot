    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                ...
                f"PERSBOT_DESKTOP_CLIENT={os.environ.get('PERSBOT_DESKTOP_CLIENT')})"
            ) from exc
...
def _classify_pip_failure(output_lines: list[str]) -> DependencyConflictError | None:
    ...
    if is_core_conflict:
        message = (
            f"检测到核心依赖版本保护冲突。{detail}插件要求的依赖版本与 Persbot 核心不兼容，"
...
class PipInstaller:
    def __init__(
        ...
        core_dist_name: str | None = "Persbot",
    ) -> None:
    ...
    async def install(
        ...
        if is_packaged_desktop_runtime():
            target_site_packages = get_persbot_site_packages_path()
