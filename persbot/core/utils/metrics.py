...
from persbot.core import db_helper, logger
...
class Metric:
    ...
    def get_installation_id():
        ...
        config_dir = os.path.join(os.path.expanduser("~"), ".persbot")
    ...
    async def upload(**kwargs) -> None:
        """上传相关非敏感的指标以更好地了解 Persbot 的使用情况。上传的指标不会包含任何有关消息文本、用户信息等敏感信息。

        Powered by TickStats.
        """
        if os.environ.get("PERSBOT_DISABLE_METRICS", "0") == "1":
            return
