...
from persbot.core.agent.handoff import HandoffTool
from persbot.core.agent.mcp_client import MCPTool
from persbot.core.agent.message import TextPart
from persbot.core.agent.tool import ToolSet
from persbot.core.astr_agent_context import AgentContextWrapper, AstrAgentContext
from persbot.core.astr_agent_hooks import MAIN_AGENT_HOOKS
from persbot.core.astr_agent_run_util import AgentRunner
from persbot.core.astr_agent_tool_exec import FunctionToolExecutor
from persbot.core.astr_main_agent_resources import (
...
from persbot.core.message.components import File, Image, Reply
from persbot.core.persona_error_reply import (
...
from persbot.core.skills.skill_manager import SkillManager, build_skills_prompt
from persbot.core.star.context import Context
from persbot.core.star.star_handler import star_map
from persbot.core.tools.cron_tools import (
    CREATE_CRON_JOB_TOOL,
    DELETE_CRON_JOB_TOOL,
    LIST_CRON_JOBS_TOOL,
)
from persbot.core.utils.file_extract import extract_file_moonshotai
from persbot.core.utils.llm_metadata import LLM_METADATAS
from persbot.core.utils.quoted_message.settings import (
        ...
        if skills:
            ...
            if runtime == "none":
                req.system_prompt += (
                    ...
                    "If you need to use these capabilities, ask the user to enable Computer Use in the Persbot WebUI -> Config."
                )
...
async def _handle_webchat(
    ...
    from persbot.core import db_helper
...
def _apply_sandbox_tools(
    ...
    if booter == "shipyard_neo":
        ...
        req.system_prompt += (
            ...
            "Preferred sequence:\n"
            "1) Use `persbot_create_skill_payload` to store canonical payload content and get `payload_ref`.\n"
            "2) Use `persbot_create_skill_candidate` with `skill_key` + `source_execution_ids` (and optional `payload_ref`) to create a candidate.\n"
            "3) Use `persbot_promote_skill_candidate` to release: `stage=canary` for trial; `stage=stable` for production.\n"
        ...
        from persbot.core.computer.computer_client import session_booter
