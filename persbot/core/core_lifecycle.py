"""Persbot 核心生命周期管理类, 负责管理 Persbot 的启动、停止、重启等操作.
...
from persbot.core.platform_message_history_mgr import PlatformMessageHistoryManager
from persbot.core.provider.manager import ProviderManager
from persbot.core.star.context import Context
from persbot.core.star.star_handler import EventType, star_handlers_registry, star_map
from persbot.core.star.star_manager import PluginManager
...
from persbot.core.updator import PersbotUpdator
from persbot.core.utils.llm_metadata import update_llm_metadata
...
class PersbotCoreLifecycle:
    ...
    def __init__(self, log_broker: LogBroker, db: BaseDatabase) -> None:
        ...
        self.persbot_config = persbot_config  # 初始化配置
        self.db = db  # 初始化数据库
    ...
    async def _init_or_reload_subagent_orchestrator(self) -> None:
        ...
        try:
            ...
            await self.subagent_orchestrator.reload_from_config(
                self.persbot_config.get("subagent_orchestrator", {}),
            )
        except Exception as e:
            logger.error(f"Subagent orchestrator init failed: {e}", exc_info=True)

    async def initialize(self) -> None:
        """初始化 Persbot 核心生命周期管理类.

        负责初始化各个组件, 包括 ProviderManager、PlatformManager、ConversationManager、PluginManager、PipelineScheduler、EventBus、PersbotUpdator等。
        """
        # 初始化日志代理
        logger.info("Persbot v" + VERSION)
        if os.environ.get("TESTING", ""):
            LogManager.configure_logger(
                logger, self.persbot_config, override_level="DEBUG"
            )
            LogManager.configure_trace_logger(self.persbot_config)
        else:
            LogManager.configure_logger(logger, self.persbot_config)
            LogManager.configure_trace_logger(self.persbot_config)
        ...
        # 初始化 Persbot 配置管理器
        self.persbot_config_mgr = PersbotConfigManager(
            default_config=self.persbot_config,
            ucr=self.umop_config_router,
            sp=sp,
        )
        self.temp_dir_cleaner = TempDirCleaner(
            max_size_getter=lambda: self.persbot_config_mgr.default_conf.get(
        ...
        try:
            await migra(
                self.db,
                self.persbot_config_mgr,
                self.umop_config_router,
                self.persbot_config_mgr,
            )
        except Exception as e:
            logger.error(f"Persbot migration failed: {e!s}")
        ...
        # 初始化人格管理器
        self.persona_mgr = PersonaManager(self.db, self.persbot_config_mgr)
        ...
        # 初始化平台管理器
        self.platform_manager = PlatformManager(self.persbot_config, self.event_queue)
        ...
        # 初始化插件管理器
        self.plugin_manager = PluginManager(self.star_context, self.persbot_config)
        ...
        # 初始化更新器
        self.persbot_updator = PersbotUpdator()
    ...
    async def start(self) -> None:
        """启动 Persbot 核心生命周期管理类.

        用load加载事件总线和任务并初始化, 执行启动完成事件钩子
        """
        self._load()
        logger.info("Persbot 启动完成。")

        # 执行启动完成事件钩子
        handlers = star_handlers_registry.get_handlers_by_event_type(
            EventType.OnPersbotLoadedEvent,
        )
        for handler in handlers:
            try:
                logger.info(
                    f"hook(on_persbot_loaded) -> {star_map[handler.handler_module_path].name} - {handler.handler_name}",
                )
    ...
    async def stop(self) -> None:
        """停止 Persbot 核心生命周期管理类, 取消所有当前任务并终止各个管理器."""
        if self.temp_dir_cleaner:
    ...
    async def restart(self) -> None:
        """重启 Persbot 核心生命周期管理类, 终止各个管理器并重新加载平台实例"""
        await self.provider_manager.terminate()
        await self.platform_manager.terminate()
        await self.kb_manager.terminate()
        self.dashboard_shutdown_event.set()
        threading.Thread(
            target=self.persbot_updator._reboot,
            name="restart",
            daemon=True,
        ).start()
    ...
    async def load_pipeline_scheduler(self) -> dict[str, PipelineScheduler]:
        ...
        """
        mapping = {}
        for conf_id, ab_config in self.persbot_config_mgr.confs.items():
    ...
    async def reload_pipeline_scheduler(self, conf_id: str) -> None:
        ...
        """
        ab_config = self.persbot_config_mgr.confs.get(conf_id)
        if not ab_config:
