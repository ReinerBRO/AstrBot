...
class _RecordEnricherFilter(logging.Filter):
    """为 logging.LogRecord 注入 Persbot 日志字段。"""
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        record.is_trace = record.name == "persbot.trace"
        return True
...
def _is_plugin_path(pathname: str | None) -> bool:
    ...
    return ("data/plugins" in norm_path) or ("persbot/builtin_stars/" in norm_path)
...
def _patch_record(record: "Record") -> None:
    ...
    extra.setdefault("persbot_version_tag", f" [v{VERSION}]" if level_no >= 30 else "")
...
class _LoguruInterceptHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        payload = {
            ...
            "persbot_version_tag": getattr(record, "persbot_version_tag", ""),
            "source_file": getattr(
                record, "source_file", _build_source_file(record.pathname)
            ),
            "source_line": getattr(record, "source_line", record.lineno),
            "is_trace": getattr(record, "is_trace", record.name == "persbot.trace"),
        }
...
class LogManager:
    _LOGGER_HANDLER_FLAG = "_persbot_loguru_handler"
    _ENRICH_FILTER_FLAG = "_persbot_enrich_filter"
    ...
    def _default_log_path(cls) -> str:
        return os.path.join(get_persbot_data_path(), "logs", "persbot.log")
    ...
    def _resolve_log_path(cls, configured_path: str | None) -> str:
        ...
        return os.path.join(get_persbot_data_path(), configured_path)
    ...
    def _setup_loguru(cls) -> None:
        ...
        cls._console_sink_id = _loguru.add(
            ...
            format=(
                ...
                "<level>[{extra[short_levelname]}]</level>{extra[persbot_version_tag]} "
    ...
    def set_queue_handler(cls, logger: logging.Logger, log_broker: LogBroker) -> None:
        ...
        handler.setFormatter(
            logging.Formatter(
                "%(ansi_prefix)s[%(asctime)s.%(msecs)03d] %(plugin_tag)s [%(short_levelname)s]%(persbot_version_tag)s "
        ...
        *,
        ...
        return _loguru.add(
            ...
            format=(
                ...
                "[{extra[short_levelname]}]{extra[persbot_version_tag]} "
    ...
    def configure_trace_logger(cls, config: dict | None) -> None:
        ...
        trace_logger = logging.getLogger("persbot.trace")
        ...
        cls._trace_sink_id = cls._add_file_sink(
            file_path=cls._resolve_log_path(path or "logs/persbot.trace.log"),
            level=logging.INFO,
            max_mb=max_mb,
            backup_count=3,
            trace=True,
