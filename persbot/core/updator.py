...
class PersbotUpdator(RepoZipUpdator):
    """Persbot 更新器，继承自 RepoZipUpdator 类
    该类用于处理 Persbot 的更新操作
    功能包括检查更新、下载更新文件、解压缩更新文件等
    """

    def __init__(self, repo_mirror: str = "") -> None:
        super().__init__(repo_mirror)
        self.MAIN_PATH = get_persbot_path()
        self.PERSBOT_RELEASE_API = "https://api.soulter.top/releases"
    ...
    def _build_frozen_reboot_args(self) -> list[str]:
        ...
        if not webui_dir:
            webui_dir = os.environ.get("PERSBOT_WEBUI_DIR")
    ...
    def _build_reboot_argv(self, executable: str) -> list[str]:
        if os.environ.get("PERSBOT_CLI") == "1":
            args = sys.argv[1:]
            return [executable, "-m", "persbot.cli.__main__", *args]
    ...
    async def check_update(
        ...
        return await super().check_update(
            self.PERSBOT_RELEASE_API,
            VERSION,
            consider_prerelease,
        )

    async def get_releases(self) -> list:
        return await self.fetch_release_info(self.PERSBOT_RELEASE_API)

    async def update(self, reboot=False, latest=True, version=None, proxy="") -> None:
        update_data = await self.fetch_release_info(self.PERSBOT_RELEASE_API, latest)
        file_url = None

        if os.environ.get("PERSBOT_CLI") or os.environ.get("PERSBOT_LAUNCHER"):
            raise Exception(
                "Error: You are running Persbot via CLI, please use `pip` or `uv tool upgrade` to update Persbot."
            )  # 避免版本管理混乱
        ...
        if latest:
            ...
            file_url = f"https://github.com/PersbotDevs/Persbot/archive/{version}.zip"
        logger.info(f"准备更新至指定版本的 Persbot Core: {version}")
        ...
        try:
            ...
            logger.info("下载 Persbot Core 更新文件完成，正在执行解压...")
