...
from persbot.core.skills.skill_manager import SANDBOX_SKILLS_ROOT, SkillManager
from persbot.core.star.context import Context
from persbot.core.utils.persbot_path import (
    get_persbot_skills_path,
...
_MANAGED_SKILLS_FILE = ".persbot_managed_skills.json"
...
def _discover_bay_credentials(endpoint: str) -> str:
    ...
    # 2. Mono-repo layout: Persbot/../pkgs/bay/credentials.json
    persbot_root = Path(__file__).resolve().parents[3]  # persbot/core/computer/ → root
    candidates.append(persbot_root.parent / "pkgs" / "bay" / "credentials.json")
...
async def _sync_skills_to_sandbox(booter: ComputerBooter) -> None:
    ...
    """
    skills_root = Path(get_persbot_skills_path())
    if not skills_root.is_dir():
        return
    local_skill_dirs = _list_local_skill_dirs(skills_root)

    temp_dir = Path(get_persbot_temp_path())
