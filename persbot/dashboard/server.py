...
from persbot.core.utils.io import get_local_ip_addresses
...
class PersbotJSONProvider(DefaultJSONProvider):
...
class PersbotDashboard:
    def __init__(
        ...
        self.config = core_lifecycle.persbot_config
        self.db = db

        # Path priority:
        # 1. Explicit webui_dir argument
        # 2. data/dist/ (user-installed / manually updated dashboard)
        # 3. persbot/dashboard/dist/ (bundled with the wheel)
        if webui_dir and os.path.exists(webui_dir):
            self.data_path = os.path.abspath(webui_dir)
        else:
            user_dist = os.path.join(get_persbot_data_path(), "dist")
        ...
        self.app.json = PersbotJSONProvider(self.app)
        ...
        self.ur = UpdateRoute(
            self.context,
            core_lifecycle.persbot_updator,
            core_lifecycle,
        )
    ...
    async def auth_middleware(self):
        ...
        if request.path.startswith("/api/v1"):
            ...
            return None
        g.username = self.config.get("dashboard", {}).get("username", "persbot")
        return None
    ...
    def run(self):
        ip_addr = []
        dashboard_config = self.core_lifecycle.persbot_config.get("dashboard", {})
        port = (
            os.environ.get("DASHBOARD_PORT")
            or os.environ.get("PERSBOT_DASHBOARD_PORT")
            or dashboard_config.get("port", 6185)
        )
        host = (
            os.environ.get("DASHBOARD_HOST")
            or os.environ.get("PERSBOT_DASHBOARD_HOST")
        ...
        ssl_enable = _parse_env_bool(
            ...
            or os.environ.get("PERSBOT_DASHBOARD_SSL_ENABLE"),
        ...
        if ssl_enable:
            cert_file = (
                ...
                or os.environ.get("PERSBOT_DASHBOARD_SSL_CERT")
                or ssl_config.get("cert_file", "")
            )
            key_file = (
                os.environ.get("DASHBOARD_SSL_KEY")
                or os.environ.get("PERSBOT_DASHBOARD_SSL_KEY")
                or ssl_config.get("key_file", "")
            )
            ca_certs = (
                os.environ.get("DASHBOARD_SSL_CA_CERTS")
                or os.environ.get("PERSBOT_DASHBOARD_SSL_CA_CERTS")
    ...
    async def shutdown_trigger(self) -> None:
        ...
        logger.info("Persbot WebUI 已经被优雅地关闭")
