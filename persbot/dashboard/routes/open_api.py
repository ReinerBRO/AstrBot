...
from persbot.core.platform.message_session import MessageSesion
...
class OpenApiRoute(Route):
    ...
    def _get_chat_config_list(self) -> list[dict]:
        conf_list = self.core_lifecycle.persbot_config_mgr.get_conf_list()
    ...
    async def get_bots(self):
        bot_ids = []
        for platform in self.core_lifecycle.persbot_config.get("platform", []):
