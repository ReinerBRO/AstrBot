...
from persbot.core.platform.sources.webchat.message_parts_helper import (
...
from persbot.core.utils.persbot_path import get_persbot_data_path, get_persbot_temp_path
...
class LiveChatRoute(Route):
    ...
    def __init__(
        ...
        self.attachments_dir = os.path.join(get_persbot_data_path(), "attachments")
        self.legacy_img_dir = os.path.join(get_persbot_data_path(), "webchat", "imgs")
    ...
    async def _unified_ws_loop(self, force_ct: str | None = None) -> None:
        """统一 WebSocket 循环"""
        username = self.config.get("dashboard", {}).get("username", "persbot")
        token = (websocket.args.get("token") or "").strip()
        if token:
            try:
                ...
                logger.warning("[Live Chat] Token expired, fallback to no-auth session")
            except jwt.InvalidTokenError:
                logger.warning("[Live Chat] Invalid token, fallback to no-auth session")
