...
from persbot.core import DEMO_MODE, logger, pip_installer
from persbot.core.config.default import VERSION
from persbot.core.core_lifecycle import PersbotCoreLifecycle
from persbot.core.db.migration.helper import check_migration_needed_v4, do_migration_v4
...
class UpdateRoute(Route):
    def __init__(
        ...
        persbot_updator: PersbotUpdator,
        ...
        self.persbot_updator = persbot_updator
    ...
    async def do_migration(self):
        ...
        try:
            ...
            await do_migration_v4(
                ...
                pim,
                self.core_lifecycle.persbot_config,
            )
    ...
    async def check_update(self):
        ...
        try:
            ...
            ret = await self.persbot_updator.check_update(None, None, False)
            return Response(
                status="success",
    ...
    async def get_releases(self):
        try:
            ret = await self.persbot_updator.get_releases()
    ...
    async def update_project(self):
        ...
        try:
            await self.persbot_updator.update(
                latest=latest,
                version=version,
                proxy=proxy,
            )
            ...
            if reboot:
                ...
                ret = (
                    Response()
                    .ok(None, "更新成功，Persbot 将在 2 秒内全量重启以应用新的代码。")
                    .__dict__
                )
                return ret, 200, CLEAR_SITE_DATA_HEADERS
            ret = (
                Response()
                .ok(None, "更新成功，Persbot 将在下次启动时应用新的代码。")
                .__dict__
            )
            return ret, 200, CLEAR_SITE_DATA_HEADERS
        except Exception as e:
