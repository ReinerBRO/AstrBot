...
from persbot.core import DEMO_MODE, file_token_service, logger
from persbot.core.core_lifecycle import PersbotCoreLifecycle
from persbot.core.star.filter.command import CommandFilter
...
from persbot.core.star.filter.regex import RegexFilter
from persbot.core.star.star_handler import EventType, star_handlers_registry
from persbot.core.star.star_manager import (
    PluginManager,
...
class PluginRoute(Route):
    ...
    async def check_plugin_compatibility(self):
        try:
            ...
            version_spec = data.get("persbot_version", "")
            is_valid, message = self.plugin_manager._validate_persbot_version_specifier(
                version_spec
            )
            return (
                Response()
    ...
    def _build_registry_source(self, custom_url: str | None) -> RegistrySource:
        """构建注册表源信息"""
        data_dir = get_persbot_data_path()
        if custom_url:
            # 对自定义URL生成一个安全的文件名
            ...
            md5_url = "https://api.soulter.top/persbot/plugins-md5"
            urls = [
                "https://api.soulter.top/persbot/plugins",
                "https://raw.githubusercontent.com/PersbotDevs/Persbot_Plugins_Collection/main/plugin_cache_original.json",
    ...
    async def install_plugin(self):
        ...
        try:
            ...
            return {
                ...
                "message": str(e),
                "data": {
                    "warning_type": "persbot_version_incompatible",
                    "can_ignore": True,
                },
            }
        except Exception as e:
