    ...
    def validate(data: dict, metadata: dict = schema, path="") -> None:
        for key, value in data.items():
            ...
            if meta["type"] == "list" and not isinstance(value, list):
                ...
                # 当前仅针对 list[dict] 的情况进行类型校验，以适配 Persbot 中 platform、provider 的配置
                for item in value:
...
def save_config(
    post_config: dict, config: PersbotConfig, is_core: bool = False
) -> None:
    """验证并保存配置"""
    errors = None
...
class ConfigRoute(Route):
    def __init__(
        ...
        core_lifecycle: PersbotCoreLifecycle,
    ) -> None:
        super().__init__(context)
        self.core_lifecycle = core_lifecycle
        self.config: PersbotConfig = core_lifecycle.persbot_config
        self._logo_token_cache = {}  # 缓存logo token，避免重复注册
        self.acm = core_lifecycle.persbot_config_mgr
        ...
        self.routes = {
            ...
            "/config/persbot/update": ("POST", self.post_persbot_configs),
    ...
    async def get_provider_template(self):
        ...
        data = {
            ...
            "providers": persbot_config["provider"],
            "provider_sources": persbot_config["provider_sources"],
        }
    ...
    async def get_configs(self):
        # plugin_name 为空时返回 Persbot 配置
        # 否则返回指定 plugin_name 的插件配置
        plugin_name = request.args.get("plugin_name", None)
        if not plugin_name:
            return Response().ok(await self._get_persbot_config()).__dict__
    ...
    async def get_embedding_dim(self):
        ...
        try:
            ...
            from persbot.core.provider.provider import EmbeddingProvider
            from persbot.core.provider.register import provider_cls_map
    ...
    async def post_persbot_configs(self):
        ...
        try:
            ...
            await self._save_persbot_configs(config, conf_id)
            await self.core_lifecycle.reload_pipeline_scheduler(conf_id)
    ...
    def _resolve_config_file_scope(
        self,
    ) -> tuple[str, str, str, StarMetadata, PersbotConfig]:
        """将请求参数解析为一个明确的配置作用域。
    ...
    async def upload_config_file(self):
        ...
        storage_root_path = Path(get_persbot_plugin_data_path()).resolve(strict=False)
    ...
    async def _get_persbot_config(self):
        config = self.config
    ...
    async def _save_persbot_configs(
        self, post_configs: dict, conf_id: str | None = None
    ) -> None:
        try:
            if conf_id not in self.acm.confs:
                raise ValueError(f"配置文件 {conf_id} 不存在")
            persbot_config = self.acm.confs[conf_id]

            # 保留服务端的 t2i_active_template 值
            if "t2i_active_template" in persbot_config:
                post_configs["t2i_active_template"] = persbot_config[
                    "t2i_active_template"
                ]

            save_config(post_configs, persbot_config, is_core=True)
        except Exception as e:
            raise e
