# 使用 Docker 部署 Persbot

> [!WARNING]
> 通过 Docker 可以方便地将 Persbot 部署到 Windows, Mac, Linux 上。
>
> 以下教程默认您的环境已安装 Docker。如果没有安装，请参考 [Docker 官方文档](https://docs.docker.com/get-docker/) 进行安装。

## 通过 Docker Compose 部署

::: details 只部署 Persbot（通用方式）

首先，需要 Clone Persbot 仓库到本地：
...
> [!TIP]
> 如果您的网络环境在中国大陆境内，上述命令将无法正常拉取。您可能需要修改 compose.yml 文件，将其中的 `image: soulter/persbot:latest` 替换为 `image: m.daocloud.io/docker.io/soulter/persbot:latest`。
:::
...
配置和使用详见 [Agent 沙盒环境](/use/persbot-agent-sandbox.md) 文档。
:::

::: details 和 NapCat 一起部署

如果您想对接 NapCat，使用这种方式可以同时部署 Persbot 和 NapCat。

```bash
mkdir persbot
cd persbot
wget https://raw.githubusercontent.com/NapNeko/NapCat-Docker/main/compose/persbot.yml
sudo docker compose -f persbot.yml up -d
```
