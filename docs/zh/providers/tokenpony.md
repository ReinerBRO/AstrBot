...
进入 Persbot WebUI，点击左栏 `服务提供商` -> `新增提供商` -> 选择 `小马算力` (需要版本 >= 4.3.3)

![配置对话模型_1](https://files.persbot.app/docs/source/images/tokenpony/image.png)
...
在 Persbot WebUI，点击左栏 `配置文件`，找到 AI 配置中的 `默认聊天模型`，选择刚刚创建的 `tokenpony`(小马算力) 提供商，点击保存。

![配置对话模型_2](https://files.persbot.app/docs/source/images/tokenpony/image_1.png)