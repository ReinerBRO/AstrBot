...
Persbot 支持接入 NewAPI 作为模型提供商，用户可以通过 NewAPI 来访问和使用各种 AI 模型服务。
...
### 在 Persbot 中配置 NewAPI 服务提供商

打开 Persbot 管理面板，进入「模型提供商」页面，然后，点击「新增模型提供商」按钮。
...
至此，您已经成功配置了 NewAPI 作为 Persbot 的模型提供商。现在，您可以通过 Persbot 来访问和使用 NewAPI 提供的各种 AI 模型服务了。
