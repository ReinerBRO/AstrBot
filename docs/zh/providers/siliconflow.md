...
进入 Persbot WebUI，点击左栏 `服务提供商` -> `新增提供商` -> 选择 `硅基流动`。

粘贴上面创建和选择的 `API Key` 和 `模型名称`，点击保存，完成创建。您可以点击下方 `服务提供商可用性` 的 `刷新` 按钮测试配置是否成功。

![配置对话模型_1](https://files.persbot.app/docs/source/images/siliconflow/image.png)

## 应用对话模型

在 Persbot WebUI，点击左栏 `配置文件`，找到 AI 配置中的 `默认聊天模型`，选择刚刚创建的 `siliconflow`(硅基流动) 提供商，点击保存。