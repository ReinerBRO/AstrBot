...
Persbot 的配置文件是一个 JSON 格式的文件。Persbot 会在启动时读取这个文件，并根据文件中的配置来初始化 Persbot，其路径位于 `data/cmd_config.json`。

> 在 Persbot v4.0.0 版本及之后，我们引入了[多配置文件](https://blog.persbot.app/posts/what-is-changed-in-4.0.0/#%E5%A4%9A%E9%85%8D%E7%BD%AE%E6%96%87%E4%BB%B6)的概念。`data/cmd_config.json` 作为默认配置文件 `default`。其他您在 WebUI 新建的配置文件会存储在 `data/config/` 目录下，以 `abconf_` 开头。
...
也可在 Persbot 日志内获取会话 ID，当一条消息没通过白名单时，会输出 INFO 级别的日志，格式类似 `aiocqhttp:GroupMessage:547540978`
...
> 此配置项仅在 `data/cmd_config.json` 中生效，Persbot 不会读取 `data/config/` 目录下的配置文件中的此项。
...
此配置项的意思是，当用户发送一张图片时，Persbot 会使用此提供商来生成对图片的描述文本，并将描述文本作为对话的上下文之一。这在对话模型不支持多模态输入时特别有用。
...
是否启用 Persbot 自带的网页搜索能力。默认为 `false`。启用后，LLM 可能会自动搜索网页并根据内容回答。
...
- `local`: 使用 Persbot 本地的文本转图像服务进行渲染。效果较差，但不依赖外部服务。
- `remote`: 使用远程的文本转图像服务进行渲染。默认使用 Persbot 官方提供的服务，效果较好。

### `t2i_endpoint`

Persbot API 的地址。用于渲染 Markdown 图片。当 `t2i_strategy` 为 `remote` 时生效。默认为空，表示使用 Persbot 官方提供的服务。
...
- `enable`: 是否启用 Persbot WebUI。默认为 `true`。
- `username`: Persbot WebUI 的用户名。默认为 `persbot`。
- `password`: Persbot WebUI 的密码。默认为 `persbot` 的 `md5` 编码值。请勿直接修改，除非您知道自己在做什么。
- `jwt_secret`: JWT 的密钥。Persbot 会在初始化时随机生成。请勿修改，除非您知道自己在做什么。
- `host`: Persbot WebUI 监听的地址。默认为 `0.0.0.0`。
- `port`: Persbot WebUI 监听的端口。默认为 `6185`。

### `platform`

> 此配置项仅在 `data/cmd_config.json` 中生效，Persbot 不会读取 `data/config/` 目录下的配置文件中的此项。

已配置的 Persbot 消息平台适配器的配置列表。
...
启用后，当请求 LLM 前，Persbot 会先发送一个预回复的表情以告知用户正在处理请求。此功能目前仅在飞书平台适配器和 Telegram 中生效。
...
唤醒前缀。默认为 `/`。当消息以 `/` 开头时，Persbot 会被唤醒。

> [!TIP]
> 如果唤醒的会话不在 ID 白名单中，Persbot 将不会响应。
...
是否启用追踪记录。默认为 `false`。启用后，Persbot 会记录运行追踪信息，可以在管理面板的 Trace 页面查看。
...
Persbot API 的基础地址。用于文件服务和插件回调等功能。如 `http://example.com:6185`。默认为空，表示不启用文件服务和插件回调功能。
