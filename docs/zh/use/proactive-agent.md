...
Persbot 引入了主动 Agent（Proactive Agent）系统，使 Persbot 不仅能被动响应用户，还能通过给自己下达未来的任务来在未来的指定时刻主动执行任务并向用户主动反馈结果（文本、图片、文件都可）。
...
- **自我唤醒**：Persbot 会在预定时间自动唤醒并执行任务。
- **任务反馈**：执行完成后，Persbot 会将结果告知任务布置方。
...
你可以在 Persbot WebUI 左侧导航栏中点击 **未来任务** 来查看和管理所有未来任务。

![](https://files.persbot.app/docs/source/images/proactive-agent/image-1.png)

### 支持的平台

“定时任务”的设置支持所有平台，然而，由于部分平台没有开放主动消息推送的 API，因此只有以下平台支持 Persbot 主动向用户推送结果：
...
为了方便 Agent 直接向用户发送图片、音频、视频等文件，Persbot 默认提供了一个 `send_message_to_user` 工具。
