...
SubAgent 编排是 Persbot 提供的一种高级 Agent 组织方式。它允许你将复杂的任务分解给多个专门的子 Agent（SubAgent）来完成，从而降低主 Agent 的 Prompt 长度，提高任务执行的成功率。
...
在 Persbot WebUI 中，点击左侧导航栏的 **SubAgent 编排**。
