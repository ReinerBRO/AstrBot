...
> [!TIP]
> 此功能目前处于技术预览阶段，可能会存在一些 Bug。如果您遇到了问题，请在 [GitHub](https://github.com/PersbotDevs/Persbot/issues) 上提交 issue。

在 `v4.12.0` 版本及之后，Persbot 引入了 Agent 沙盒环境，以替代之前的代码执行器功能。沙盒环境给 Agent 提供了更安全、更灵活的代码执行和自动化操作能力。

![](https://files.persbot.app/docs/source/images/persbot-agent-sandbox/image.png)

## 启用沙盒环境

目前，沙盒环境仅支持通过 Docker 来运行。我们目前使用了 [Shipyard](https://github.com/PersbotDevs/shipyard) 项目作为 Persbot 的沙盒环境驱动器。未来，我们会支持更多类型的沙盒环境驱动器，如 e2b。

## 性能要求

Persbot 给每个沙盒环境限制最高 1 CPU 和 512 MB 内存。

我们建议您的宿主机至少有 2 个 CPU 和 4 GB 内存，并开启 Swap，以保证多个沙盒环境实例可以稳定运行。

### 使用 Docker Compose 部署 Persbot 和 Shipyard

如果您还没有部署 Persbot，或者想更换为我们推荐的带沙盒环境的部署方式，推荐使用 Docker Compose 来部署 Persbot，代码如下：
...
这会启动一个包含 Persbot 主程序和沙盒环境的 Docker Compose 服务。

### 单独部署 Shipyard

如果您已经部署了 Persbot，但没有部署沙盒环境，可以单独部署 Shipyard。
...
> [!TIP]
> 如果您使用 Docker 部署 Persbot，您也可以修改上面的 Compose 文件，将 Shipyard 的网络与 Persbot 放在同一个 Docker 网络中，这样就不需要暴露 Shipyard 的端口到宿主机。

## 配置 Persbot 使用沙盒环境

> [!TIP]
> 请确保您的 Persbot 版本在 `v4.12.0` 及之后。

在 Persbot 控制台，进入 “配置文件” 页面，找到 “Agent 沙箱环境”，启用沙箱环境开关。
