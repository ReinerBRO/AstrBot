...
2. **得到客服账号名。** 在 `客服账号` 中创建一个客服账号，记录下名称，填入 Persbot 配置的 `微信客服账号名` 中（不是账号 ID）。

3. **得到企业 ID。** 在 [企业微信 - 企业信息](https://work.weixin.qq.com/wework_admin/frame#profile) 得到企业 ID（`Corpid`），复制到 Persbot 配置的 `corpid` 处。
...
点击下方的两个随机获取，得到 `Token` 和 `EncodingAESKey`，复制到 Persbot 配置的 `token` 和 `encoding_aes_key` 处。请保持 `统一 Webhook 模式 (unified_webhook_mode)` 为开启状态。然后点击保存配置，等待适配器加载完成。

回调 URL 填写：

- 如果开启了 `统一 Webhook 模式`，点击保存之后，Persbot 将会自动为你生成唯一的 Webhook 回调链接，你可以在日志中或者 WebUI 的机器人页的卡片上找到，将该链接填入回调 URL 处。
...
> 请注意放行端口。如果开启了统一 Webhook 模式，需要将请求转发到 Persbot 所在服务器的 `6185` 端口；如果没有开启，则转发到配置指定的端口（默认 `6195`）。
...
> [!TIP]
> 根据 [#571](https://github.com/Soulter/Persbot/issues/571) 的反馈，对于新注册的企业，`corp_id` 可能要注册一段时间后才生效（前后大概过了半个小时）。
...
点击 `我的企业`，查看并得到企业 ID（`Corpid`），复制到 Persbot 配置的 `corpid` 处。
...
进入应用，查看并得到机器人的 `Secret`，复制到 Persbot 配置的 `secret` 处。
...
并且点击下方的两个随机获取，得到 `Token` 和 `EncodingAESKey`，复制到 Persbot 配置的 `token` 和 `encoding_aes_key` 处。建议保持 `统一 Webhook 模式 (unified_webhook_mode)` 为开启状态。

现在应该已经填完 Persbot 连接到企业微信的所有配置项。点击 Persbot 配置页右下角保存，等待 Persbot 重启。
...
如果能够保存成功，Persbot 就已经能够接收信息。

## 测试

在企业微信-工作台中，找到刚刚创建的应用，发送 `/help`，看看 Persbot 是否能够回复。

![image](https://files.persbot.app/docs/source/images/wecom/3dc9fa61145ab0dd8f56a10295affec8_720.png)
