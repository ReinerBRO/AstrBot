# Connect Persbot to WeChat Official Account Platform

Persbot supports WeChat Official Account integration (version >= v3.5.8). After setup, you can chat with Persbot directly in the WeChat Official Account chat interface.
...
![Development Interface Management](https://files.persbot.app/docs/source/images/weixin-official-account/image.png)

Copy AppID and AppSecret from WeChat platform to Persbot fields `appid` and `secret`.

Open IP whitelist and add your public IP(s), one per line if multiple.

In server configuration, click modify.

- `Token`: create any string with length 3-32, and fill the same value in Persbot `token`.
- `EncodingAESKey`: click random generate and fill Persbot `encoding_aes_key`.

Keep `Unified Webhook Mode (unified_webhook_mode)` enabled (recommended), then save Persbot config and wait for restart.

For `URL`:

- If unified mode is enabled, use the unique callback URL generated by Persbot (from logs or bot card).
...
> - Unified mode enabled: forward to Persbot port `6185`
...
Scan it with WeChat, send `help`, and check whether Persbot replies.
...
Persbot supports custom endpoint via `api_base_url` for environments without stable public IP.
