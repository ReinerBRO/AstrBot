...
# After installation, the data directory is fixed at: ~/.local/share/persbot
```

# Starting
>[!TIP]
> You can directly use `persbot init` (for the first run) to initialize.
> Use `persbot run` to run the bot.
...
```bash
systemctl --user start persbot.service
```

# Auto-start on Boot
```bash
# For security reasons, it is designed to run as a user.
systemctl --user enable persbot.service
# If you need to start it immediately, add --now
# systemctl --user enable --now persbot.service
```
