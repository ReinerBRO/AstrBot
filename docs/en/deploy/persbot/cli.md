# Deploy Persbot from Source Code
...
```bash
git clone https://github.com/PersbotDevs/Persbot.git
...
# git clone --depth=1 --branch $(git ls-remote --tags --sort='-v:refname' https://github.com/PersbotDevs/Persbot.git | head -n1 | awk -F/ '{print $3}') https://github.com/PersbotDevs/Persbot.git
cd Persbot
```
...
![image](https://files.persbot.app/docs/source/images/cli/image.png)
...
2. Execute in terminal (in the Persbot directory)
```bash
uv sync
uv run main.py
```
...
In the Persbot source code directory, run the following command in the terminal:
...
> ![image](https://files.persbot.app/docs/source/images/cli/image-1.png)
...
If there are no errors, you will see a log message similar to `🌈 Dashboard started, accessible at` with several links. Open one of the links to access the Persbot dashboard. The link is `http://localhost:6185`.

> [!TIP]
> If you are deploying Persbot on a server, you need to replace `localhost` with your server's IP address.
>
