...
# Persbot Configuration File

## data/cmd_config.json

Persbot's configuration file is a JSON format file. Persbot reads this file at startup and initializes based on the settings within. Its path is `data/cmd_config.json`.

> Since Persbot v4.0.0, we introduced the concept of [multiple configuration files](https://blog.persbot.app/posts/what-is-changed-in-4.0.0/#%E5%A4%9A%E9%85%8D%E7%BD%AE%E6%96%87%E4%BB%B6). `data/cmd_config.json` serves as the default configuration `default`. Other configuration files you create in the WebUI are stored in the `data/config/` directory, starting with `abconf_`.

The default Persbot configuration is as follows:
...
Session IDs can also be found in Persbot logs; when a message fails the whitelist, an INFO level log is output, e.g., `aiocqhttp:GroupMessage:547540978`.
...
> This item only takes effect in `data/cmd_config.json`; Persbot does not read this from configuration files in the `data/config/` directory.
...
This means when a user sends an image, Persbot uses this provider to generate a text description, which is then used as part of the conversation context. This is useful when the conversation model doesn't support multimodal input.
...
Whether to enable Persbot's built-in web search capability. Default is `false`. When enabled, the LLM may automatically search the web and answer based on the content.
...
- `local`: Uses Persbot's local T2I service for rendering. Lower quality but doesn't depend on external services.
- `remote`: Uses a remote T2I service for rendering. Uses the official Persbot service by default, which offers better quality.

### `t2i_endpoint`

Persbot API address. Used for rendering Markdown images. Effective when `t2i_strategy` is `remote`. Default is empty, meaning the official Persbot service is used.
...
- `enable`: Whether to enable the Persbot WebUI. Default is `true`.
- `username`: Username for the Persbot WebUI. Default is `persbot`.
- `password`: Password for the Persbot WebUI. Default is the `md5` encoded value of `persbot`. Do not modify directly unless you know what you are doing.
- `jwt_secret`: JWT secret key. Persbot generates this randomly at initialization. Do not modify unless you know what you are doing.
- `host`: Address the Persbot WebUI listens on. Default is `0.0.0.0`.
- `port`: Port the Persbot WebUI listens on. Default is `6185`.

### `platform`

> This item only takes effect in `data/cmd_config.json`; Persbot does not read this from configuration files in the `data/config/` directory.

List of configured Persbot message platform adapter settings.
...
When enabled, Persbot sends a pre-reply emoji before requesting the LLM to inform the user that the request is being processed. This currently only takes effect in the Lark and Telegram platform adapters.
...
Wake prefix. Default is `/`. When a message starts with `/`, Persbot is awakened.

> [!TIP]
> If the awakened session is not in the ID whitelist, Persbot will not respond.
...
Whether to enable trace recording. Default is `false`. When enabled, Persbot records execution traces, which can be viewed on the Trace page of the admin panel.
...
Base address for the Persbot API. Used for file services, plugin callbacks, etc. E.g., `http://example.com:6185`. Default is empty, meaning file services and plugin callbacks are disabled.
